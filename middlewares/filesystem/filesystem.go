...
var (
	...
	// 与WithBackend、WithShell、WithStreamingShell中设置等价
	WithOperator = einolib.MakeOption(func(c *FileSystemMiddlewareConfig, v einolib.Operator) {
		c.Backend = v
		c.Shell = v
		c.StreamingShell = v
	})
...
func NewFileSystemMiddleware(ctx context.Context, fileSystemMiddlewareConfig *FileSystemMiddlewareConfig) (adk.ChatModelAgentMiddleware, error) {
	if fileSystemMiddlewareConfig.StreamingShell != nil && fileSystemMiddlewareConfig.Shell != nil {
		fileSystemMiddlewareConfig.Shell = nil
	}
...
func init() {
	if err := einolib.RegisterMiddlewareConstructFunc(einolib.MiddlewareTypeFileSystem, einolib.MiddlewareNameGeneral, createFileSystemMiddleware, (*FileSystemMiddlewareConfig)(nil)); err != nil {
		einolib.GetLogger().Errorf("register middleware %s failed: %v", einolib.MiddlewareTypeFileSystem, err)
	}
}
